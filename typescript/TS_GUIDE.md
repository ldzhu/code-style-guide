...
* 使用点和横杠来分割文件名(用’-’来分割单词,用’.’来分割类型)
  ...
  ```typescript
  /* 不推荐*/
  columnSelector.component.ts
  columnSelectorComponent.test.ts
...
* 相关功能的文件其根文件名部分保持一致
    ...
    // task-manage.component.ts
    @Component({
        selector: 'otn-task-manage',
        templateUrl: './task.component.html',
        styleUrls: ['./task-style.component.scss']
    })
...
* 类名以类型结尾
  ...
  ```typescript
  /* 不推荐*/
  export class AutoInspect {...}
  export class ParameterSetting {...}
  export class Task {...}
  
  /* 推荐*/
  // 模块
  export class AutoInspectModule {...}
  // 组件
  export class ParameterSettingComponent {...}
  // 服务
...
* 命名需直观
  ...
  ```typescript
  /* 不推荐*/
  public gridData1: GridDataResult;
  public gridData2: GridDataResult;
  
  /* 推荐*/
  public gridData: GridDataResult;
  public originalGridData: GridDataResult;
  ```
